...
public class EventCreationController {
    ...
    public String addEventData(@RequestBody EventCreation event) {
        ...
        try {
            ...
            //Specify sql: for vent table insertion
            String addEventSql = """
            ...
            /*If newEventRow insertion was > 0 (meaning 1 row inserted) & a key was retrieved
            ...
            if (newEventRow > 0 && eventKeyHolder.getKey() != null) {
                ...
                //Insert a parking row for each created event (helps track passes left by event)
                ...
                //passesAvailable for each show is constant & 57 is fictitious for now (leave value for now)
                ...
                /*For each event, insert one record for every row the venue holds, into rowsbyevent table
                with composite pk rowid, eventid (from static rows table, and the event the row is being tracked for) */
                String eventRowsSql = """
                            INSERT INTO public.rowsbyevent (rowid, eventid, ticketsleft, soldout)
    ...
    public String updateEvent(@PathVariable int id, @RequestBody EventCreation event) {
        ...
        int rowsAffected = jdbcTemplate.update(updateEventSql, event.getTitle(), event.getDescription(), event.getEventDate(), event.getEventTime(), event.getVenueid(), id);
